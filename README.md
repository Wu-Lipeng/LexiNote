...
2. 点击右上角齿轮，选择“从文件安装插件”，选择 `lexinote-0.1.1.xpi`。
...
插件内置了 BingeWord 使用的百度词典链路。选择“百度文本翻译·词典版”后填写百度 API Key 和 Secret Key，插件会请求 `https://aip.baidubce.com/oauth/2.0/token` 获取 access_token，并在内存中缓存；按接口返回的有效期提前一小时刷新。随后调用 `https://aip.baidubce.com/rpc/2.0/mt/texttrans-with-dict/v1`，发送 `from=en`、`to=zh` 和选中的单词，解析 `trans_result[0].dict` 中的 `word_result.simple_means`、`edict`、音标和例句。

百度 API Key 和 Secret Key 使用 Zotero 凭据存储；插件不会把 Secret Key 写进通用模板或笔记。百度接口返回的词性、释义、音标和例句会合并成浮窗内容，并按普通文字写入笔记。
...
当前版本手动分发，尚未部署自动更新服务器。清单中 Zotero 要求的更新地址使用保留的 `.invalid` 域名；自动检查更新不会获取新版本，需要手动安装新 XPI。
