...
2. 点击右上角齿轮，选择“从文件安装插件”，选择 `lexinote-0.2.3.xpi`。
...
通用版可启用内置试用密钥。试用请求按本机 Zotero 配置文件每天最多 50 次，设置页会显示当天剩余额度。该限制是本地机制，清除或修改本地数据可以绕过；内置密钥也会随 XPI 一起分发，只应使用专门的低额度试用应用。

发布前，在 `addon/trial-credentials.js` 中填写该试用应用的 `baiduApiKey` 与 `baiduSecretKey`；不要填写日常或高额度应用的凭据。
